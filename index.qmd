...
Full conditional posterior distribution could be estimated base on Bayes' theorem and priors shown above. Therefore, the full conditional posterior distribution is the proportion to the product of likelihood functions and priors.
$$
\begin{aligned}
...
\end{align}
$$
Then, we could have joint posterior distribution of $\tau,\tau_{0},\sigma^{2},\sigma_{\eta}^{2}$:
$$
\begin{align}
P(\tau,\tau_{0},\sigma^{2},\sigma_{\eta}^{2}|\mathbf{y}) &\propto P(\mathbf{y}|\tau,\tau_{0},\sigma^{2},\sigma_{\eta}^{2})\times P(\tau,\tau_{0},\sigma^{2},\sigma_{\eta}^{2})\\
&= L(\tau,\tau_{0},\sigma^{2},\sigma_{\eta}^{2}|\mathbf{y})\times P(\tau,\tau_{0},\sigma^{2},\sigma_{\eta}^{2})
\end{align}
$$

Therefore, the full Conditional posterior distribution of $\boldsymbol\tau$ is derived as follows:
$$
\begin{align}
P(\boldsymbol\tau|\mathbf{y},\tau_{0},\sigma^{2},\sigma_{\eta}^{2})&\propto L(\boldsymbol\tau|\mathbf{y},\tau_{0},\sigma^{2},\sigma_{\eta}^{2}) \space P(\boldsymbol\tau|\tau_{0},\sigma_{\eta}^{2})\\
...
\\ \overline{V_{\tau_{0}}}&=\left[ \sigma_{\eta}^{-2}\mathbf{I}_T +\underline{V_{\tau_{0}}}^{-1}\right]^{-1} \\
...
UC.Gibbs.sampler    = function(S, starting.values, priors){
  ...
  for (s in 1:S){
    ...
    tau_0.bar      = V.tau_0.bar %*% ( (1/aux$sigma[1])*t(i)%*%priors$H%*%aux$tau + tau_0.v.inv*priors$tau_0 )
