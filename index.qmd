...
UC.local.tau.sigma.Gibbs.sampler    = function(S, starting.values, priors){
  ...
  for (s in 1:S){
    ...
    V.tau.bar = solve(V.tau.bar.inv)
    V.tau.bar = 0.5*(V.tau.bar + t(V.tau.bar))
    sigma.s   = as.numeric(priors$sigma.s + t(aux$Y)%*%aux$Y - t(aux$Y)%*%V.tau.bar%*%aux$Y)
