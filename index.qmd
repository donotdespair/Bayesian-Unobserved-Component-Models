...
$$
s = \Gamma(\frac{\underline{\mu}}{2})^{-1} (\frac{\underline{s}}{2})^{\frac{\underline{\mu}}{2}} s^{-\frac{\underline{\mu}+2}{2}}exp\{ -\frac{1}{2}\frac{\underline{s}}{s}\}
$$

As we assume $\sigma^2|s \sim \mathcal{IG2} (s,\underline{\nu})$

$$
\sigma^2|s = \Gamma(\frac{\underline{\nu}}{2})^{-1} (\frac{s}{2})^{\frac{\underline{\nu}}{2}} \sigma^2{^{-\frac{\underline{\nu}+2}{2}}}exp\{-\frac{1}{2}\frac{s}{\sigma^2}\}
$$
...
In order to obtain a sample from the posterior distribution we use a Gibbs sampler. We generate random draws from the joint posterior distribution and we update them at each iteration. Therefore we exploit the following procedure:

Initialize $s$ at $s^{(0)}$.

At each iteration s

- 1. Draw $(\tau, \sigma^2)^{(s)} \sim p(y,X, \tau^{(s-1)}, \sigma^{2(s-1)}, s^{(s)})$

- 2. Draw $s^{(s)} \sim p(s|\tau^{(s)}, \sigma^{2(s)})$

Repeat steps 1 and 2 for $(S_1+S_2)$ times. Discard the first $S_1$ repetitions. Return the output as $\{\tau^{(s)}, \sigma^{2(s)}, s^{(s)}\}^{S_1+S_2}_{s = S_1+1}$
...
#| message: false
#| warning: false
sigma2       = 1 # this part need to draw from before steps, and here just an example of it
...
s.sampler = function (total_S, prior){ 
  # total_S iteration of sampler for s
  #
  # total_S is the iteration times
  #
  ...
  # define posterior
  s.posterior  = matrix(NA, total_S, 1)
  # initial value of lambda
  s.posterior[1] = 10                            # set s0
  
  
  # --------------------- s.sampler --------------------- 
  # Update parameters for s posterior
  a.bar.s      = 1/sigma2
  b.bar.s      = (prior$nu.prior - prior$mu.prior)/2
  p.bar.s      = prior$s.prior

  # Draw next period value for s from GIG distribution
  if (s!=total_S){
    s.posterior[s+1]      = GIGrvg::rgig(n = 1, lambda = p.bar.s, chi = b.bar.s, psi = a.bar.s)
    }
  }
  output                  = list(s.posterior = s.posterior) 
  # for Gibb sampler needs to discard first S1 times: s.posterior[(S1+1):S2,]
