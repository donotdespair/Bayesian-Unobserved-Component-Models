...
Therefore, by $\Sigma = D^{-1}$, we could transform multivariate normal distribution into:

$$
N_{T}(D^{-1}b,D^{-1})
$$

This parameterisation is particularly useful as matrices $D$ and $b$ can be computed easily. Additionally, matrix $D$ that in the simulation smoother for state-space models is a band matrix and often a tridiagonal matrix. Therefore, the computation time is much shorter than usual ways. For tridiagnal matrix $D$ in state space model, let $L$ be a lower-triangular matrix obtained by Cholesky decomposition, where $D = LL'$ (Assume that $L^{-1}$ can be compute efficiently).
...
Therefore, we could clearly see that the taking time drop a lot due to simulation smoothing method.
