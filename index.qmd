...
from which we recognize a GIG distribution $\mathcal{GIG}(\overline{p}, \overline{a}, \overline{b})$ with parameters:
...
The following script illustrates sampling from the full conditional posterior distribution of $s$ and use function `s.sampler`:
...
s.sampler = function (total_S){ 
  for (s in 1:total_S){
    
  # Update parameters for s posterior
  a.bar.s      = 1/sigma2
  b.bar.s      = (nu.prior - mu.prior)/2
  p.bar.s      = s.prior
