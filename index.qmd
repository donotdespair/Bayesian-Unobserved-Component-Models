...
A simple Unobserved Component model with drift
$$\tau_{t} = \mu + \tau_{t-1} + \eta_t,\\$$
$$\eta_t|Y_{t-1} ~ iid \sim\mathcal{N}(0, \sigma_{\eta}^2)\\$$

Unobserved Component model with time-varying drift parameter
$$\tau_{t} = \mu_{s_t} + \tau_{t-1} + \eta_t,\\$$
$$\eta_t|Y_{t-1} ~ iid \sim\mathcal{N}(0, \sigma_{\eta}^2)\\$$

$$\mu_{s_t} :\text{regime-dependent mean term}$$

We assume there are two regime states, for example, before and after the exchange rate crisis in 1985.
Therefore

$$s_t = 1 \text{, for t < Tb}$$
$$s_t = 2 \text{, for t} \geq Tb$$
\begin{align}
\xi_{t} = \begin{bmatrix} 
1 \\
0 \\
\end{bmatrix},  \quad \text{for } s_t = 1 \\
\xi_t = \begin{bmatrix} 
0 \\
1 \\
\end{bmatrix}, \quad \text{for } s_t = 2 \\
\end{align}


Matrix form
$$
H\tau = X_{\tau}\beta_{s}+\eta\\
$$
Prior distribution
$$p(\beta) = exp\left(-\frac{1}{2}(\beta - \underline{\beta})'\underline{V}_\beta^{-1}(\beta - \underline{\beta})\right)$$

Full conditional posterior distribution
$$ p(\beta|y,\tau,\sigma_\eta^2) = L(\beta|y,\tau,\sigma_\eta^2)p(\beta) = N_2(\overline{\beta},\overline{V_\beta})\\$$ 
$$ \overline{V_{\beta}} = [\sigma_{\eta}^{-2}X_\tau'X_\tau+ \underline{V}_\beta^{-1}]^{-1}\\$$ 
$$ \bar{\beta} = \overline{V_{\beta}}[\sigma_{\eta}^{-2}X_\tau'H\tau + \underline{V}_\beta^{-1}\underline{\beta}]$$ 

Since the drift is time varying, 
\begin{align}
\beta = \begin{bmatrix}
\tau_0 \\
\mu_1 \\
...
draw_beta <- function(X_tau, H, tau, sigma_eta_sq, beta_prior, V_prior) {
