...
lambda_t <- function(Y,X,A.posterior,Sigma.posterior){
  lambda.nu.prior = 5
  N = ncol(Y)
  t = nrows(Y)
  for (x in 1:t) {
    u_t <- Y[x,] - t(X[x,]) %*% A.posterior
    lambda.posterior <- rinvgamma(1, (N + lambda.nu.prior)/2, (lambda.nu.prior + u_t %*% Sigma.posterior %*% t(u_t))/2)
  }
}
```
