...
And the Joint prior distribution:

```{=tex}
\begin{aligned}

P(\tau,\tau_{0},\sigma^{2},\sigma_{\eta}^{2})=P(\tau|\tau_{0},\sigma_{\eta}^{2})P(\tau_{0}|\sigma_{\eta}^{2})P(\sigma_{\eta}^{2})P(\sigma^{2})

\end{aligned}
```

We could have **Full Conditional posterior distribution** for $\tau,\tau_{0},\sigma^{2},\sigma_{\eta}^{2}$ based on two likelihood functions:

Full Conditional posterior distribution for $\tau$:

```{=tex}
\begin{aligned}

P(\tau|y,\tau_{0},\sigma^{2},\sigma_{\eta}^{2})&=L(\tau|y,\tau_{0},\sigma^{2},\sigma_{\eta}^{2})P(\tau|\tau_{0},\sigma_{\eta}^{2})\\
&\propto exp\left\{ -\frac{1}{2}\sigma^{-2}\left( y-\tau \right)'I_{T}\left( y-\tau \right) \right\}\\
&\times exp\left\{ -\frac{1}{2}\sigma_{\eta}^{-2}\left(\tau-H^{-1} i\tau_{0}\right)'H'H\left( \tau-H^{-1} i\tau_{0} \right) \right\}

\\where: P(\tau|y,\tau_{0},\sigma^{2},\sigma_{\eta}^{2})\sim N(\bar{\tau},\bar{V_{\tau}})\\
\bar{V_{\tau}}&=\left[ \sigma^{-2}I_{T} +\sigma_{\eta}^{-2}H'H\right]^{-1}\\
\bar{\tau}&=\bar{V_{\tau}}\left[ \sigma_{\eta}^{-2}H'i\tau_{0} +\sigma^{-2}I_{T}y\right]

\end{aligned}
```

Full Conditional posterior distribution for $\tau_{0}$:

```{=tex}
\begin{aligned}

P(\tau_{0}|y,\tau,\sigma^{2},\sigma_{\eta}^{2})&=L(\tau_{0}|y,\tau,\sigma^{2},\sigma_{\eta}^{2})P(\tau_{0}|\sigma_{\eta}^{2})\\
&\propto exp\left\{ -\frac{1}{2}\sigma_{\eta}^{-2}\left(H\tau -i\tau_{0}\right)'I_{T}\left(H\tau-i\tau_{0}\right)\right\}\\
&\times exp\left\{ -\frac{1}{2}\left(\tau_{0}-  \underline{\tau}_{0}\right)'\underline{V}_{\tau_{0}}^{-1}\left(\tau_{0}-  \underline{\tau}_{0}\right) \right\}

\\where: P(\tau_{0}|y,\tau,\sigma^{2},\sigma_{\eta}^{2})\sim N(\bar{\tau_{0}},\bar{V_{\tau_{0}}})\\
\bar{V_{\tau_{0}}}&=\left[ \sigma_{\eta}^{-2}I_{T} +\underline{V}_{\tau_{0}}^{-1}\right]^{-1}\\
\bar{\tau_{0}}&=\bar{V_{\tau_{0}}}\left[ \sigma_{\eta}^{-2}I_{T}H\tau i +\underline{V}_{\tau_{0}}^{-1}\underline{\tau}_{0}\right]

\end{aligned}
```

Full Conditional posterior distribution for $\sigma_{\eta}^{2}$:

```{=tex}
\begin{aligned}

P(\sigma_{\eta}^{2}|y,\tau,\tau_{0},\sigma^{2})&=L(\sigma_{\eta}^{2}|y,\tau,\tau_{0},\sigma^{2})P(\sigma_{\eta}^{2})
\\
&\propto \left( \sigma_{\eta}^{2} \right)^{-\frac{T}{2}}exp\left\{ -\frac{1}{2}\sigma_{\eta}^{-2}\left(H\tau -i\tau_{0}\right)'I_{T}\left(H\tau-i\tau_{0}\right)\right\}
\\
&\times \left( \sigma_{\eta}^{2} \right)^{-\frac{\underline{\upsilon}+2}{2}}exp\left\{ -\frac{1}{2}\frac{\underline{s}}{\sigma_{\eta}^{2}}\right\}
\\
where: P(\sigma_{\eta}^{2}|y,\tau,\tau_{0},\sigma^{2})\sim IG2(\bar{s_{\eta}},\bar{\upsilon_{\eta}})
\\
\bar{s_{\eta}}&=\underline{s} + \left(H\tau -i\tau_{0}\right)'I_{T}\left(H\tau-i\tau_{0}\right)
\\

\end{aligned}
```

Full Conditional posterior distribution for $\sigma^{2}$:

```{=tex}
\begin{aligned}

P(\sigma^{2}|y,\tau,\tau_{0},\sigma_{\eta}^{2})&=L(\sigma^{2}|y,\tau,\tau_{0},\sigma_{\eta}^{2})P(\sigma^{2})
\\
&\propto \left( \sigma^{2} \right)^{-\frac{T}{2}} exp\left\{ -\frac{1}{2}\sigma^{-2}\left(y-\tau\right)'I_{T}\left(y-\tau\right)\right\}
\\
&\times \left( \sigma^{2} \right)^{-\frac{\underline{\upsilon}+2}{2}}exp\left\{ -\frac{1}{2}\frac{\underline{s}}{\sigma^{2}}\right\}
\\
where: P(\sigma^{2}|y,\tau,\tau_{0},\sigma_{\eta}^{2})\sim IG2(\bar{s_{\sigma}},\bar{\upsilon_{\sigma}})
\\
\bar{s_{\sigma}}&=\underline{s} + \left(y-\tau\right)'I_{T}\left(y-\tau\right)
\\
\bar{\upsilon_{\sigma}}&= \underline{\upsilon}+T

\end{aligned}
```

Therefore, we could have basic Gibbs Sampler method:



The following function is for Gibbs sampler for posterior:
```{r Gibbs Sampler}
#| echo: true
